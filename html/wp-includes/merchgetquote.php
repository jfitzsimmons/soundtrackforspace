...
foreach ($getidsx as $key4=>$value4) {
    ...
    foreach ($varis as $key2=>$value2) {
        foreach ($f2 as $key3=>$value3) {
            ...
            if (($value4[$key4] == $f2[$key3][post_id]) && ($f2[$key3][meta_key] = 'scalable') && ($f2[$key3][meta_value] = 1)){

              
                $firephp->log($f2[$key3], ‘Object‘);
                ...
                if ($i < $maxiterations) {
                    ...
                    if ($keystr === 'attribute_pa_color') {
                        ...
                        echo "v4 ". $value4[$key4] ."</br></br>";
                        echo "postid ". $f2[$key3][post_id] ."</br></br>";
                ...
                if ($i3 < $maxiterations) {
                    if ($keystr === 'shirtname') {
                        echo "shirt </br></br>";
                        array_push($shirtnames, $value3[meta_value]);
                        $paramkey="items[".$key4."][products][". $i3 ."][id]";
                        $params[$paramkey] = (string)$value3[meta_value];
                        $i3++;
                    }
                }
